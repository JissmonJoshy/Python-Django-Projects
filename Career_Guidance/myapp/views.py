...
def clgapprove(request):
    id = request.GET['id']
    clg_user = Login.objects.get(id=id)
    clg_user.is_active = True
    clg_user.save()

    college = College.objects.get(user=clg_user)
    
    send_mail(
        'College Registration Approved',
        f'Hello {college.name},\n\nYour college registration has been approved. You can now log in and access the platform.\n\nThank you.',
...
def clgreject(request):
    ...
    Login.objects.filter(id=college.user.id).delete()
    college.delete()
...
def update(request):
    id = request.GET['id']
    clg = Course.objects.get(id=id)  # get single Course instance
    if request.method == "POST":
        name = request.POST['name']
        duration = request.POST['duration']
        fees = request.POST['fees']
        details = request.POST['details']
        gpa = float(request.POST['gpa'])  # Ensure GPA is a float
    ...
    return render(request, "college/update.html", {'ups': [clg]})
...
def test(request):
    ...
    # Assign 10 random questions if not already assigned
    if 'assigned_questions' not in request.session:
        questions = list(Question.objects.order_by('?')[:10])
        assigned_qna_primary_keys = [q.id for q in questions]
        request.session['assigned_questions'] = assigned_qna_primary_keys
        request.session['obj_index'] = 0
        request.session['qcount'] = 1
    else:
    ...
    if request.method == 'POST':
        ...
        if question.answer == selected:
            user_answer, created = Answer.objects.get_or_create(std=user, defaults={'one': 0})
        ...
        if curr_obj_index < len(assigned_qna_primary_keys):
            curr_obj = Question.objects.get(pk=assigned_qna_primary_keys[curr_obj_index])
        else:
            # Clear session and redirect to the results page after the test
        ...
        if curr_obj_index < len(assigned_qna_primary_keys):
            ...
            # Clear session and redirect to user home if test is completed
            del request.session['assigned_questions']
            del request.session['obj_index']
            del request.session['qcount']
...
def eligible_course(student_id):
    ...
    eligible_colleges = College.objects.filter(course__in=eligible_courses)
...
def apply_to_college(request):
    if request.method == 'POST':
        ...
        already_applied = CollegeApplication.objects.filter(student=student, college=college, course=course).exists()
        if already_applied:
            messages.warning(request, 'You have already applied to this course.')
        else:
            CollegeApplication.objects.create(student=student, college=college, course=course)
            messages.success(request, 'Application submitted successfully.')

        return redirect('eligible_colleges')  
...
def viewLoan(request):
    ...
    return render(request,'financer/viewLoan.html',{'data':data})
...
import chatgui  # Import the script you want to run
...
def show_questions(request):
    ...
    return render(request, 'admin/show_questions.html', {'questions': questions})
...
def show_jobs(request):
    ...
    return render(request, 'admin/show_jobs.html', {'jobs': jobs})
...
def edit_job(request, id):
    ...
    if request.method == "POST":
        # job.course_id = request.POST['course']
    ...
    return render(request, 'admin/edit_job.html', {'job': job, 'courses': courses})
...
def viewMentors(request):
    uid = request.session['Uid']
    clg = College.objects.get(user_id=uid)
    mentors = Mentor.objects.filter(clg=clg)
    return render(request, 'college/viewMentors.html', {'mentors': mentors})
...
def editMentor(request, id):
    ...
    if request.POST:
        ...
        login = Login.objects.get(id=mentor.user.id)
    ...
    return render(request, 'college/editMentor.html', {'mentor': mentor})
...
def view_applied_colleges(request):
    ...
    student = Student.objects.get(user_id=uid)
    applications = CollegeApplication.objects.filter(student=student)
    return render(request, 'student/view_applied_colleges.html', {'applications': applications})
...
def view_applications_college(request):
    uid = request.session.get('Uid')
    college = College.objects.get(user_id=uid)
    applications = CollegeApplication.objects.filter(college=college)
    return render(request, 'college/view_applications_college.html', {'applications': applications})


def update_application_status(request, app_id, status):
    CollegeApplication.objects.filter(id=app_id).update(status=status)
    return redirect('view_applications_college')
...
def apply_job(request, job_id):
    if 'Uid' in request.session:
        ...
        return render(request, 'student/apply_job.html', {'job': job, 'student': student})
...
def view_applied_jobs(request):
    ...
    student = Student.objects.get(user=login_obj)
    applications = JobApplication.objects.filter(student=student)
    return render(request, 'student/view_applied_jobs.html', {'applications': applications})


def admin_view_applications(request):
    applications = JobApplication.objects.select_related('student', 'job')
    return render(request, 'admin/admin_view_applications.html', {'applications': applications})


    
def admin_view_college_applications(request):
    applications = CollegeApplication.objects.select_related('student', 'college', 'course')
    return render(request, 'admin/admin_view_college_applications.html', {'applications': applications})
...
def view_all_courses(request):
    data = Course.objects.all()
    return render(request, 'college/view_all_courses.html', {'data': data})